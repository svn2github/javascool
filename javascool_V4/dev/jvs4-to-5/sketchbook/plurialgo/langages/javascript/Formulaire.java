...
public class Formulaire {
	...
	public void constrFormu(Programme prog, StringBuffer buf, int indent) {		
		...
		Divers.ecrire(buf,"document.writeln(" + prog.quote("</table>") + "); ", indent);
//		for (Iterator<raffinat.inout.langages.modele.Argument> iter=instr_pere.arguments.iterator(); iter.hasNext();) {
//			Argument arg = (Argument) iter.next();
...
// -------------------------------
	...
	private void constrLabelFormu(Programme prog, StringBuffer buf, int indent, String msg, Argument arg) {
		...
		if (msg!=null) {
			Divers.ecrire(buf,"document.writeln(" + prog.quote("<td>" + msg + "</td>") + "); ");
		}
	}
	...
	private void constrSimpleFormu(Programme prog, StringBuffer buf, int indent, String msg, Argument arg) {
		...
		if (arg.isEntier() || arg.isReel()) {
			...
			Divers.ecrire(buf,"document.writeln(" + prog.quote("<td>" + txt + "</td>") +  "); ");
		}
		else if (arg.isTexte()) {
	...
	private void constrTexteFormu(Programme prog, StringBuffer buf, int indent, String msg, Argument arg) {
		...
		if (arg.avecTexteRadio()) {
			...
			int n = liste.length;
			String txt;
			Divers.ecrire(buf,"document.writeln(" + prog.quote("<td>") +  "); ", indent);
			for(int i=0; i<n; i++) {
				txt = "<input type='radio' name='" + zone + "' value='" + liste[i] + "'>" + liste[i] + "</input>";
				Divers.ecrire(buf,"document.writeln(" + prog.quote(txt) +  "); ", indent);
				if (i<n-1) Divers.ecrire(buf,"document.writeln(" + prog.quote("<br>") +  "); ", indent);
			}
			Divers.ecrire(buf,"document.writeln(" + prog.quote("</td>") +  "); ", indent);
		}
			...
			int n = liste.length;
			String txt;
			txt= "<select size='2' " + "name='" + zone + "'>";
			Divers.ecrire(buf,"document.writeln(" + prog.quote("<td>" + txt) +  "); ", indent);
			for(int i=0; i<n; i++) {
				txt = "<option value='" + liste[i] + "'>" + liste[i] + "</option>";
				Divers.ecrire(buf,"document.writeln(" + prog.quote(txt) +  "); ", indent);
			}
			Divers.ecrire(buf,"document.writeln(" + prog.quote("</select></td>") +  "); ", indent);
		}
		else {
			this.constrLabelFormu(prog, buf, indent, msg, arg);	
			String txt = "<input type='text' size='8' " + "name='" + zone + "'></input>";
	...
	private void constrTabFormu(Programme prog, StringBuffer buf, int indent, String msg, Argument arg, String indice) {
		...
		Divers.ecrire(buf,"document.writeln(" + prog.quote("</tr>") + "); ", indent);
		Divers.ecrire(buf,"document.writeln(" + prog.quote("</table></td>") + "); ", indent);
	}
	...
	private void constrMatFormu(Programme prog, StringBuffer buf, int indent, String msg, Argument arg) {
		...
		Divers.ecrire(buf,"document.writeln(" + prog.quote("<td><table>") + "); ", indent);
		Divers.ecrire(buf, "for(i1=0; i1<" + prog.getMaxTab() + "; i1++) {", indent);
		Divers.ecrire(buf,"document.writeln(" + prog.quote("<tr>") + "); ", indent+1);
	...
	public void constrClasseFormu(Programme prog, StringBuffer buf, int indent, String msg, Argument arg) {
	...
	private void constrTabClasseFormu(Programme prog, StringBuffer buf, int indent, String msg, Argument arg) {
		this.constrLabelFormu(prog, buf, indent, msg, arg);
		// la boucle
		Divers.ecrire(buf,"document.writeln(" + prog.quote("<td><table border='1'>") + "); ", indent);
...
// -------------------------------
	...
	private void lireSimpleFormu(Programme prog, StringBuffer buf, int indent, Argument arg) {
		String zone = transformerMsg("zone_" + arg.nom);
		if (arg.isEntier()) {
			Divers.ecrire(buf, arg.nom + " = eval( formu[" + prog.quote(zone) + "].value ); ", indent);
		}
		else if (arg.isReel()) {
			Divers.ecrire(buf, arg.nom + " = eval( formu[" + prog.quote(zone) + "].value ); ", indent);
		}
			...
			Divers.ecrire(buf, arg.nom + " = formu[" + prog.quote(zone) + "].checked; ", indent); 
		}
	}
	...
	private void lireTexteFormu(Programme prog, StringBuffer buf, int indent, Argument arg) {
		...
		if (arg.avecTexteRadio()) {
			...
			for(int i=0; i<n; i++) {
				Divers.indenter(buf, indent);
				Divers.ecrire(buf, "if (formu[" + prog.quote(zone) + "][" + i + "].checked) "); 
				Divers.ecrire(buf, arg.nom + " = formu[" + prog.quote(zone) + "][" + i + "].value;");
			}
		}
		else {
			Divers.ecrire(buf, arg.nom + " = formu[" + prog.quote(zone) + "].value; ", indent); 
		}
	}
