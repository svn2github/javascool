...
public class ProgletEngine {
	...
	try {
		...
		try {
		    ...
		    System.err.println("Erreur lors de la détection dans le jar de la proglet " + name + " en " + dir + " ("
			    + e + ")");
		}
	    }
	} catch (Exception er) {
	...
	if (getProglet() != null) {
	    ...
	    jvs2java.setProgletPackageName(getProglet().hasFunctions() ? "org.javascool.proglets." + getProglet().getName()
		    : null);
	}
