...
public class Proglet {
     ...
     * 
     * @param location
     *            L'URL (Universal Resource Location) où se trouve la proglet.
	...
	try {
	    pml.set("jvs-translator", Class.forName("org.javascool.proglets." + pml.getString("name") + ".Translator")
		    .newInstance());
	} catch (Throwable e) {
	}
	return this;
	...
	if (!pml.isDefined("pane-defined")) {
		...
		try {
		    Component pane = (Component) Class.forName("org.javascool.proglets." + pml.getString("name") + ".Panel")
			    .newInstance();
