...
public class HtmlDisplay extends JPanel {
	...
	}
	{
	    ...
	    JScrollPane spane = new JScrollPane(pane, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
	...
	try {
	    ...
	    setText("Cette page est à l'adresse internet: <tt>«" + location.replaceFirst("^(mailto):.*", "$1: ...")
	...
	if (location.startsWith(HtmlDisplay.stringPrefix)) { // Affichage de
	    ...
	    org.javascool.main.Desktop.getInstance().openBrowserTab(url.toString(),
	...
	try {
	    return urls.isEmpty() ? Macros.getResourceURL(location) : urls.getCurrent() instanceof URL ? new URL(
