...
class TextFilesEditor extends TabbedPane {
    ...
    public TextFilesEditor(){
	super();
	if(Desktop.getInstance().getOlderState()!=null){
	    for(String file:Desktop.getInstance().getOlderState().getOpenFiles()){
		if(FileManager.exists(file)){
		    try {
			this.openFile(false, file);
		    } catch (Exception e) {} // Passage sous tapis, on ouvre pas d'onglet
		}
	    }
	}
    }
	...
	return this.openFile(dialog, null);
    }
    
    private TextFileEditor openFile(boolean dialog,String fileUrl) {
	TextFileEditor file = new TextFileEditor().setExtension(extension);
	if(fileUrl!=null){
	    file.load(fileUrl, true);
	    if(canOpen(file)){
	...
	for (int i = 0; ok && i < getTabCount(); i++) {
	    if (getComponentAt(i) instanceof TextFileEditor
		    && file.getFileLocation().equals(((TextFileEditor) getComponentAt(i)).getFileLocation())) {
		ok = false;
	    }
	}
	if (!ok) {
	    int r = JOptionPane.showConfirmDialog(null, "Le fichier " + file.getName()
		    + " est déjà ouvert, voulez-vous vraiment éditer «sur» l'autre version ?", "Erreur d'ouverture",
	...
	if (getSelectedIndex() != -1 && getComponentAt(getSelectedIndex()) instanceof TextFileEditor)
	    ...
	    if (verbose) {
		JOptionPane
			.showMessageDialog(null, "Il n'y a aucun fichier sélectionné !", "Erreur", JOptionPane.ERROR_MESSAGE);
	    }
	    return null;
	}
    }
