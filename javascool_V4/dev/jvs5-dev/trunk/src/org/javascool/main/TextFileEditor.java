...
class TextFileEditor extends TextEditor {
	...
	String name = location == null ? "Nouveau Fichier" :
	    location.replaceAll(".*/", "");
    ...
    private String extension = "jvs"; // @todo Use by default txt or jvs ?
	...
	try {
		...
		JOptionPane.showMessageDialog(null,
			"Le fichier ne peut pas être lu, il est inaccessible en tant que fichier texte.", "Erreur de lecture",
	 ...
	 * fc.showOpenDialog(null); if (returnVal ==
	 ...
	 * fc.setDialogTitle("Enregistrer"); int returnVal =
	 * fc.showOpenDialog(this.getParent()); if (returnVal ==
	 * JFileChooser.APPROVE_OPTION) {
	 * TextFileEditor.setWorkingDir(fc.getSelectedFile().getParentFile()
	 * .getAbsolutePath()); save(fc.getSelectedFile().getAbsolutePath());
	...
	if (isTextModified()) {
		...
		try {
			...
			if (lastModified != 0 && l != 0 && lastModified < l) {
			    int r = JOptionPane.showConfirmDialog(null, "Le fichier " + getName()
		    ...
		    JOptionPane.showMessageDialog(null, "Le fichier ne peut pas être écrit ici, choisisez un nouvel endroit.",
    ...
    @Override
    public String toString() { // @edit Change XML output by a JSON output
	//return "<TextFileEditor location=\"" + location + "\" name=\"" + getName() + "\"/>";
	JSONObject json=new JSONObject();
	json.put("type", "TextFileEditor");
