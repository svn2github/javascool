...
public class Pml {
	...
	if ("xml".equals(format))
	    return reset(Xml2Xml.run(value, Pml.xml2pml).replaceAll("¨", "\\\\\"").replaceAll("«", "\\{"), "pml");
	else if ("htm".equals(format) || "html".equals(format))
	    return reset(Xml2Xml.run(Xml2Xml.html2xhtml(value), Pml.xml2pml), "pml");
	else {
	    reset(value);
	}
	return this;
	...
	format = format.toLowerCase();
	return "xml".equals(format) ? new XmlWriter().toString(this) : "raw".equals(format) ? new PlainWriter().toString(this,
		0) : "php".equals(format) ? new PhpWriter().toString(this) : "jmf".equals(format) ? new JmfWriter()
		.toString(this) : new PlainWriter().toString(this, 180);
    }
	...
	public void check(boolean ok, String message) {
	    if (!ok) {
		System.out.println("Erreur de syntaxe \"" + message + "\", ligne "
			+ (itoken < tokens.size() ? "" + tokens.get(itoken).line + " vers \"" + getToken(0) + "\"" : "finale"));
	    }
	}
	...
	private boolean write2d(Pml pml, int n, int tab) {
	    ...
	    if (pml.getSize() == 0) {
		boolean ln = n >= 0
			&& (n == 0 || (pml.getParent() != null && pml.getParent().getChild(n - 1) != null && pml.getParent()
				.getChild(n - 1).getSize() > 0));
		writeln(ln, tab);
	...
	private static String quote(String string) {
	    return string == null ? "null" : string.matches("[a-zA-Z_][a-zA-Z0-9_]*") || "\"{\"".equals(string)
	...
	private void write(Pml pml) {
		...
		string.append(" ").append(
			pml.getTag().replaceFirst("^\"([{}])\"$", "$1").replaceAll("&", "&amp;").replaceAll("<", "&lt;"));
	    } else {
		...
		for (String name : pml.attributes()) {
		    string.append(" ")
			    ...
			    .append("=\"")
			    .append(pml.getChild(name).toString().replaceFirst("^\\{(.*)\\}$", "$1")
				    .replaceFirst("^\"(.*)\"$", "$1").replaceAll("&", "&amp;").replaceAll("<", "&lt;")
				    .replaceAll("\"", "&quot;")).append("\"");
		}
		if (pml.getCount() > 0) {
		    string.append(">");
	...
	public String toString(Pml pml) {
		...
		for (String name : pml.attributes()) {
		    string.append(", ").append(PhpWriter.quote(name)).append(" => ")
	...
	if (usage.length > 0) {
	    new Pml().load(usage[0]).save(
		    ...
		    (usage.length > 1 && usage[1].matches(".*\\.(pml|php|xml|jmf)")) ? usage[1].replaceFirst(".*\\.", "")
			    : "pml");
	}
    }
}
