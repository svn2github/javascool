...
// -------------------------------
	...
	private void lireSimpleStandard(Programme prog, StringBuffer buf, int indent, String msg) {
		if (this.isTexte()) {
			...
			Divers.ecrire(buf, this.nom + " = eval(Input( " + transformerMsg(msg) + " )); ", indent); 
		}
		else {
			Divers.ecrire(buf, this.nom + " = Number(Input( " + transformerMsg(msg) + " )); ", indent); 
		}
	}
...
//	 --------------------------------
	...
	private void ecrireExpression(Programme prog, StringBuffer buf, int indent) {
		...
		Divers.ecrire(buf, "Println(" + this.nom + "); ");
	}
