...
public class Main extends JApplet {
  ...
  private void initGUI() {
    ...
    try { setActivityAs(getParameter("activity"));
    } catch(Exception e) {}
    try { setFileAs(getParameter("file"));
    } catch(Exception e) {}
  }
  ...
  private abstract class ProcessingActivity extends JavaActivity {
    ...
    private void initApplet() {
      ...
      try {
        ...
        try { width = (Integer) Class.forName(name).getDeclaredField("WIDTH").get(null);
        } catch(Throwable e) {
          System.err.println("Width undefined !");
          width = 800;
        }
        try { height = 20 + (Integer) Class.forName(name).getDeclaredField("HEIGHT").get(null);
        } catch(Throwable e) {
          System.err.println("Height undefined !");
          height = 600;
        }
        if(popup) {
  ...
  private abstract class AlgoEditorActivity extends JavaActivity {
    ...
    public void init() {
      if(algoEditor == null)
        algoEditor = new AlgoEditor();
      if(algoViewer == null) {
        algoViewer = new JvsSourceEditor();
        algoViewer.reset(false);
      }
  ...
  private JvsSourceEditor algoViewer = null;
