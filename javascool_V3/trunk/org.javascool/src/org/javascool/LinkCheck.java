...
public class LinkCheck {
  ...
  private static void check(String location) {
    try {
      String text = Utils.loadString(location);
      npages++;
              ...
              if(!isReadable(href))
                ...
                if(anchor != null) {
                  if(!anchors.containsKey(href))
                    anchors.put(href, getAnchorSet(Utils.loadString(href)));
  ...
  public static boolean isReadable(String location) {
    ...
    } catch(IOException e) { throw new IllegalArgumentException("Bad URL " + location + " (" + e + ")");
    }
  }
