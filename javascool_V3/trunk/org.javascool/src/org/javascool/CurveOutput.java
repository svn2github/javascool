...
public class CurveOutput extends JPanel implements Widget {
  ...
  private static class point { double x, y; }; private Vector<Vector<point>> curves = new Vector<Vector<point>>();
  private static class line { double x1, y1, x2, y2; Color c; } private Vector<line> lines = new Vector<line>();
  private static class oval { double x, y, w, h; Color c; } private Vector<oval> ovals = new Vector<oval>();
  private static class label { double x, y; String s; Color c; } private Vector<label> labels = new Vector<label>();
  {
  ...
  /**/public void paint(Graphics g) {
    width = getWidth(); height= getHeight(); i0 = width/2; j0 = height/2; w0 = i0 - 10; h0 = j0 - 10;
    super.paint(g);
    paintBackground(g);
    g.setPaintMode(); 
    for(int c = 0; c < 10; c++) {
  ...
  private int x2i(double x) { return  (int) Math.rint(i0 + w0 * x); }
  private int y2j(double y) { return  (int) Math.rint(j0 - h0 * y); }
  private int x2w(double x) { return  (int) Math.rint(w0 * x); }
  private int y2h(double y) { return  (int) Math.rint(h0 * y); }
  private double i2x(int i) { return  ((double) (i - i0)) / w0; }
  private double j2y(int j) { return  ((double) (j0 - j)) / h0; }
  ...
  private class ReticuleMouseListener implements MouseMotionListener, MouseListener {
    ...
    public void mouseDragged(MouseEvent e) {
      ...
      reticuleX = i2x(e.getX()); reticuleY = j2y(e.getY());
      paintReticule(getGraphics());
    }
  ...
  public void add(double x, double y, int c) {
    point p = new point(); p.x = x / Xscale; p.y = y / Yscale;
    if (0 <= c && c < 10)
      curves.get(c).add(p);
    repaint(0, 0, getWidth(), getHeight());
  }
  ...
  public void add(double x1, double y1, double x2, double y2, int c) {
    line l = new line(); l.x1 = x1 / Xscale; l.y1 = y1 / Yscale; l.x2 = x2 / Xscale; l.y2 = y2 / Yscale; l.c = 0 <= c && c < 10 ? colors[c] : Color.BLACK;
    lines.add(l);
  }
  ...
  public void add(double x, double y, double r, int c) {
    oval l = new oval(); l.x = (x - r) / Xscale; l.y = (y + r) / Yscale; l.w = 2 * r/ Xscale; l.h = 2 * r / Yscale; l.c = 0 <= c && c < 10 ? colors[c] : Color.BLACK;
    ovals.add(l);
  }
  ...
  public void add(double x, double y, String s, int c) {
    label l = new label(); l.x = x / Xscale; l.y = y / Yscale; l.s = s; l.c = 0 <= c && c < 10 ? colors[c] : Color.BLACK;
    labels.add(l);
  }

  /** Gets the horizontal reticule position. */
  public double getReticuleX() { return Xscale * reticuleX; }

  /** Gets the vertical reticule position. */
  public double getReticuleY() { return Yscale * reticuleY; }
