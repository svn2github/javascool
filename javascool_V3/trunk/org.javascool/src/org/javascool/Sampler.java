...
public abstract class Sampler {
   ...
   */
  public void start(int samplingPeriod) { if (samplingPeriod > 0) this.samplingPeriod = samplingPeriod; start(); }
  /**/public void start() {
    error = null;
   ...
   */
  public void setSamplingPeriod(int samplingPeriod) { if (samplingPeriod > 0) this.samplingPeriod = samplingPeriod; } private int samplingPeriod = 10;

  /** Pauses/Resumes the iteration mechanism. */
  public void resume(boolean resume) { this.resume = resume; }

  // Throws a runtime-exception if any
  private void check() { Thread.yield(); if (error != null) { RuntimeException e = error; error = null; throw new RuntimeException("Sampling exception: " + e); } }
   ...
   */
  public void stop() { loop = resume = false; check(); } 
   ...
   * @throws RuntimeException if a runtime exception has occurred during sampling.  
   */
  public int getSpareTime() { check(); return spareTime; } private int spareTime = -Integer.MIN_VALUE;
}
