...
public class GogleMapParcours {
	...
	static void parcoursRec(GogleMapPanel g, Set < String > vu, String ville1) {
		...
		for (String ville2 : g.arcs.get(ville1)) afficheVille(g,ville2);
		Macros.sleep(500);
		for (String ville2 : g.arcs.get(ville1)) if ( ! vu.contains(ville2)) parcoursRec(g,vu, ville2);
	}
		...
		while ( ! aVoir.isEmpty()) {
			...
			for (String ville2 : g.arcs.get(ville1)) 
				if ( ! vu.contains(ville2)) {
					...
					if (!aVoir.contains(ville2)) aVoir.offer(ville2);
				}
		}
	}
