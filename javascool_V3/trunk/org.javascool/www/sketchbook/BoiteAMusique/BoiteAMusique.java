...
class Radar{
  ...
  public void drawBody()
  {
    ...
    while(i<5) {
      ...
      if (i==1) {
        w=8*sw; h=2*sw; xcoo=bx-bs/2-w*3/4; ycoo=by-sw;            
      } else if (i==2) {
        w=2*sw; h=8*sw; xcoo=bx-sw; ycoo= by+(bs/2)-h*1/4;          // h = largeur des balises; w = longueur des balises
      } else if (i==3) {
        w=2*sw; h=8*sw; xcoo=bx-sw; ycoo= by-(bs/2)-h*3/4; 
      } else {
        w=8*sw; h=2*sw; xcoo=bx+bs/2-w*1/4; ycoo=by-sw; 
      }
    ...
    for(int i=0;i<freqs.tab[0].length;i++) {  
      ...
      if(mySource[1]==i) {
        ...
        if(mySource[0]==0) {
          ...
          if(i<freqs.tabE.length) text(" " + freqs.tabE[i] + " " , sources[2][i].x, sources[2][i].y);
