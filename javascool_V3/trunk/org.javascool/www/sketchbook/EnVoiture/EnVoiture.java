...
public class EnVoiture extends PApplet {
  ...
  */
  ...
  public void setup() 
  {
    ...
    colors[0] = color(0xffFF9900); colors[1] = color(100, 200,0); colors[2] = color(0xffFFFF00);
    colors[3] = color(200); colors[4] = color(150); colors[5] = color(100);
    colors[6] = color(0xff0000FF);  colors[7] = color(0,0,100);  colors[8] = color(0xff00FFFF);
    colors[9] = color(0xffFF00FF); colors[10] = color(100, 0, 100); colors[11] = color(0,70,75);
  ...
  public void draw() 
  {
    ...
    for(String ni_ : (Iterable<String>) myTrip.spots.keySet()) {
      ...
      if(start!=null && end!=null) {
        ...
        else {strokeWeight(1.1f); stroke(0);}
      }
...
class Trip {
  ...
  public String exploreSpot(String sO, String sInit, String sTarget, ArrayList Visited)                            
  {
    ...
    for(String ni_ : (Iterable<String>) spots.keySet()) {
      if(!(Visited_.contains(ni_))) spotsN.put(ni_, (Spot) spots.get(ni_)); // contraindre \u00e0 chercher parmi les spots non visit\u00e9s!
    }
