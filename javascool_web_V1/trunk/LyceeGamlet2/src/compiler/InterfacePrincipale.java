...
/*	static Konsol getKonsol(){
	...
	String filePath =  "..\\src\\compilation\\Ctest.java";
		 FileWriter fw = new FileWriter(filePath);
	     fw.write(getJProgramEditorPane().getText());
	     fw.close();
		///filePath =  "Ctest.java";
		//String filePath2 = "Ctest.bin";
	       String filePath2 = "..\\bin\\compilation\\Ctest.bin";
	       deleteClassFile(  new File("..\\bin\\compilation\\Ctest.class") );
	      Compile.run(filePath, filePath2);
	       
	    deplacer(new File("..\\src\\compilation\\Ctest.class"),new File("..\\bin\\compilation\\Ctest.class"));
	
		

	 final Class s=ToolProvider.getSystemToolClassLoader().loadClass(getParameter("proglet"));
	 ///progletM = "main"
	 //proglet= "compilation.Ctest"
	
		 new Thread(new Runnable(){public void run(){
			
			 try {
				s.getDeclaredMethod(getParameter("progletM")).invoke(null);
			} catch (IllegalArgumentException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			} catch (SecurityException e) {
