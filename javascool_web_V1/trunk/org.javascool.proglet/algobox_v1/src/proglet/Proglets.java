...
static public JPanel getPanel(String proglet) 
{
    ...
    try
 {
 return (JPanel) Class.forName("proglet."+proglet).getField("panel").get(null);
 }
 
    catch(Exception e)
 {
 System.err.println(e+" (unkown proglet "+proglet+")");
 return new JPanel();
 }
   ...
   * @param file The icon file name.
 The icon must be located in the <tt>img/</tt> directory (directory on the server or on the client side or in the jar).
  ...
  static ImageIcon getIcon(String file)
 {
    ...
    try {
 return new ImageIcon(InterfacePrincipale.class.getClassLoader().getResource("img/"+file));
 }
 catch(Exception e1) {
      ...
      try {
 System.out.println("Warning: loading "+file+" via gforge");

	return new ImageIcon(new URL("http://javascool.gforge.inria.fr/proglet/img/"+file));
 } catch(Exception e2)
 {
  ...
  static public void test(String proglet)
 {
    ...
    try {
 Class.forName("proglet."+proglet).getDeclaredMethod("test").invoke(null);
 } catch(Exception error) { }

  }

  private static String toUcfirst(String string)
 {
  ...
  static void report(Throwable error)
 {
  ...
  public static void main(String usage[]) {
    ...
    String proglet = usage.length == 2 ? usage[1] : usage.length == 1 ? usage[0] : "Konsol";
 applet.setProglet(proglet);
 
    boolean edit = usage.length == 2 ? "edit".equals(usage[0]) : true;
 applet.setEdit(edit);
 
    try { if (edit && usage.length == 3) applet.doLire(usage[2]);
 } catch(Exception e)
 {
 ...
 */
  ...
  static final String proglets[] = new String[]
 {
 "Konsol", "Dicho", "Smiley", "Scope", "Conva", "Synthe", "Tortue"
 };

  private static int getProgletIndex(String proglet) {
 for(int i = 0; i < proglets.length; i++)
 if (proglets[i].equals(proglet)) return i;
 return -1;
 }
  ...
  Use <tt>frame.dispose()</tt> to close the frame.
